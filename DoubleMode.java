...
public class DoubleMode implements GameModeController {
    ...
    // 用于切换难度后更新内部 logic 引用（在 GameWindow 中会重新创建对象，所以不需要额外方法）
    ...
    private void reveal() {
        ...
        if (cell.isMine()) {
            audio.playExplode();
            cell.setRevealed(true); cell.setExploded(true);
        ...
        outer: for (int r=0; r<logic.getRows(); r++)
            for (int c=0; c<logic.getCols(); c++)
                if (!logic.getCell(r,c).isMine() && !logic.getCell(r,c).isRevealed()) { win=false; break outer; }
        if (win) {
            audio.playWin();
    ...
    private void endRound() {
        ...
        if (player1.getRoundsPlayed() >= GameConfig.MAX_ROUNDS && player2.getRoundsPlayed() >= GameConfig.MAX_ROUNDS) {
            gameEnded = true;
            String win = player1.getTotalScore() > player2.getTotalScore() ? "红方" : (player2.getTotalScore() > player1.getTotalScore() ? "蓝方" : "平局");
        ...
        logic.generateMines();
        cursorRow = logic.getRows()/2; cursorCol = logic.getCols()/2;
        window.setCursor(cursorRow, cursorCol);
    ...
    public void updateUIComponents(GameWindow w) {
        ...
        String t1 = player1.isActive() ? String.format("👑 回合 %d/%d | 本轮:%d", player1.getRoundsPlayed()+1, GameConfig.MAX_ROUNDS, player1.getRoundScore()) : String.format("⏳ 回合 %d/%d", player1.getRoundsPlayed(), GameConfig.MAX_ROUNDS);
        String t2 = player2.isActive() ? String.format("👑 回合 %d/%d | 本轮:%d", player2.getRoundsPlayed()+1, GameConfig.MAX_ROUNDS, player2.getRoundScore()) : String.format("⏳ 回合 %d/%d", player2.getRoundsPlayed(), GameConfig.MAX_ROUNDS);
    ...
    public void reset() {
        player1.resetForNewGame(); player2.resetForNewGame();
        currentPlayer = player1; player1.setActive(true);
        roundActive = true; gameEnded = false;
        logic.resetBoardState(); logic.generateMines();
        cursorRow = logic.getRows()/2; cursorCol = logic.getCols()/2;
        window.setCursor(cursorRow, cursorCol);
        updateUI();
    }

    private void updateUI() { window.updateUI(); }
}